...
def get_transforms(img_size, use_dropout=False):
    train_aug_list = [
...
def parse_args():
    ...
    parser.add_argument('--train-img', type=str, default='lq5/train')
    parser.add_argument('--train-mask', type=str, default='mask_gt/train')
    parser.add_argument('--val-img', type=str, default='lq5/val')
    ...
    # 【変更】保存パスを half_channels 実験用に変更
    parser.add_argument('--save-path', type=str, default='Kuzushiji_Restoration/experiments/segmentation/ablation/half_channels/best.pth')
    parser.add_argument('--log-path', type=str, default='Kuzushiji_Restoration/experiments/segmentation/ablation/half_channels/train.log')
    ...
    # アブレーション用のフラグ (デフォルトFalseなので、フラグを付けなければ無効)
    parser.add_argument('--use-dropout', action='store_true', help='Use CoarseDropout augmentation')
    ...
    # 【変更】デフォルトで 0.5 の Dropout を適用するように変更
    parser.add_argument('--head-dropout', type=float, default=0.0, help='Dropout rate before segmentation head (0.0 to disable)')

    # 【追加】デコーダーチャンネルを半分にするフラグ (デフォルトTrue)
    parser.add_argument('--half-decoder-channels', action='store_true',  help='Use half decoder channels')
...
def main():
    ...
    # Run名の自動生成
    extra_tags = []
    if args.use_dropout: extra_tags.append("Dropout")
    if args.use_scse: extra_tags.append("scSE")
    
    # 【追加】Depthがデフォルト(5)でない場合、名前に反映

    # 【追加】Head Dropoutが有効な場合、名前に反映
    if args.head_dropout > 0: extra_tags.append(f"HeadDrop{args.head_dropout}")
    
    # 【追加】Half Channelsが有効な場合、名前に反映
    if args.half_decoder_channels: extra_tags.append("HalfCh")
    ...
    train_t, val_t = get_transforms(args.image_size, use_dropout=args.use_dropout)
    ...
    encoder_params = dict(
        ...
        in_channels=3, 
        classes=1,
        encoder_depth=args.encoder_depth  # 引数で指定された深さを使用
    )
    
    decoder_attention_type = 'scse' if args.use_scse else None

    # 【重要】Depthに応じたデコーダーチャンネル調整
    if args.encoder_depth == 4:
        # Depth 4用のデコーダーチャンネル
        if args.half_decoder_channels:
            encoder_params["decoder_channels"] = (128, 64, 32, 16)
        else:
            encoder_params["decoder_channels"] = (256, 128, 64, 32)
    elif args.encoder_depth == 5:
        # Depth 5用のデコーダーチャンネル (デフォルト)
        if args.half_decoder_channels:
            encoder_params["decoder_channels"] = (128, 64, 32, 16, 8)
        else:
            encoder_params["decoder_channels"] = (256, 128, 64, 32, 16)

    logging.info(f"Model Config: Depth={args.encoder_depth}, DecoderAttn={decoder_attention_type}, HalfCh={args.half_decoder_channels}")
    ...
    # 【追加】Segmentation Headの前にDropout2dを挿入
    if args.head_dropout > 0:
        logging.info(f"Inserting Dropout2d(p={args.head_dropout}) before segmentation head.")
