...
from basicsr.models.archs.SwinIR_arch import swinir_tiny


def load_model(checkpoint_path, img_size=128, device='cuda'):
    """
    SwinIRモデルをロード
    ...
    """
    # モデルを構築
    model = swinir_tiny(img_size=img_size)
...
def main():
    parser = argparse.ArgumentParser(description='SwinIR推論スクリプト (No Mask)')
    parser.add_argument(
        '--checkpoint',
        type=str,
        default=None,
    ...
    # モデルをロード
    print("\n=== モデルのロード ===")
    model = load_model(args.checkpoint, img_size=args.img_size, device=args.device)
    ...
    for img_path in tqdm(image_files, desc="処理中"):
        try:
            # 画像を処理
            restored_img = process_image(model, img_path, device=args.device)
