# ------------------------------------------------------------------------
# SwinIR Training Script for Kuzushiji Restoration
# Based on BasicSR training framework
# ------------------------------------------------------------------------
import argparse
import datetime
import logging
    ...
    if (opt['logger'].get('wandb')
        ...
        if wandb_run:
            wandb.config.update({
                ...
                'in_chans': opt['network_g'].get('in_chans', 3),
                'embed_dim': opt['network_g'].get('embed_dim', 60),
                'depths': opt['network_g'].get('depths', [6, 6, 6, 6, 6, 6]),
                'num_heads': opt['network_g'].get('num_heads', [6, 6, 6, 6, 6, 6]),
                'window_size': opt['network_g'].get('window_size', 8),
                'optimizer': opt['train']['optim_g']['type'],
                'learning_rate': opt['train']['optim_g']['lr'],
                'batch_size': opt['datasets']['train']['batch_size_per_gpu'],
                'total_iters': opt['train'].get('total_iter', 'N/A'),
            })
    tb_logger = None
...
def main():
    ...
    if opt['logger'].get('log_params', False) or opt['logger'].get('log_flops', False):
        try:
            ...
            in_chans = opt['network_g'].get('in_chans', 3)
            img_size = opt['network_g'].get('img_size', 128)
            
            dummy_input = torch.randn(1, in_chans, img_size, img_size).to(model.device)
            ...
            logger.info(f'  Input size: {in_chans} x {img_size} x {img_size}')
            ...
            # Save to text file
            stats_file = osp.join(opt['path']['experiments_root'], 'model_stats.txt')
            with open(stats_file, 'w') as f:
                f.write(f'Model: {opt["network_g"]["type"]}\n')
                f.write(f'Input size: {in_chans} x {img_size} x {img_size}\n')
                f.write(f'Parameters: {params_str} ({params:,})\n')
                f.write(f'MACs: {macs_str} ({macs:,})\n')
                f.write(f'FLOPs: {flops_str} ({flops:,})\n')
            logger.info(f'Model stats saved to: {stats_file}')
            ...
            if wandb_run:
                ...
                wandb.run.summary['model_params_raw'] = params
                wandb.run.summary['model_macs'] = macs_str
                wandb.run.summary['model_flops'] = flops_str
                wandb.run.summary['model_flops_G'] = flops / 1e9
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        while train_data is not None:
            ...
            if current_iter % opt['logger']['print_freq'] == 0:
                ...
                # Log all losses to wandb (save_all_losses option)
                if wandb_run and opt['logger'].get('save_all_losses', True):
                    import wandb
                    wandb_log = {
                        'train/epoch': epoch,
    ...
    if wandb_run:
        import wandb
        wandb.run.summary['training_time'] = consumed_time
        wandb.run.summary['total_iters'] = total_iters
