...
class ImageRestorationModel(BaseModel):
    ...
    def init_training_settings(self):
        ...
        if train_opt.get('tv_opt'):
            tv_type = train_opt['tv_opt'].pop('type')
            cri_tv_cls = getattr(loss_module, tv_type)
            self.cri_tv = cri_tv_cls(**train_opt['tv_opt']).to(self.device)
        else:
            self.cri_tv = None

        # Edge-Aware Loss for character stroke sharpness
    ...
    def dist_validation(self, dataloader, current_iter, tb_logger, save_img, rgb2bgr, use_image):
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            # Handle mask for metrics
            mask_img = None
            if 'mask' in visuals:
                mask_img = tensor2img([visuals['mask']], rgb2bgr=False) # H,W or H,W,1
                ...
                if use_image:
                    for name, opt_ in opt_metric.items():
                        ...
                        self.metric_results[name] += getattr(
                            metric_module, metric_type)(sr_img, gt_img, mask=mask_img, **opt_)
                else:
                    for name, opt_ in opt_metric.items():
                        metric_type = opt_.pop('type')
                        self.metric_results[name] += getattr(
                            metric_module, metric_type)(visuals['result'], visuals['gt'], mask=mask_img, **opt_)
        ...
        # Only use distributed reduce if distributed training is initialized
    ...
    def get_current_visuals(self):
        ...
        # Extract mask from LQ if 4-channel
        if self.lq.shape[1] == 4:
             out_dict['mask'] = self.lq[:, 3:4, :, :].detach().cpu()
        return out_dict
