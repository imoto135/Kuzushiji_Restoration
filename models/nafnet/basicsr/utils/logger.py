...
class MessageLogger():
    ...
    def __init__(self, opt, start_iter=1, tb_logger=None):
        ...
        # total_iter と total_epoch のどちらかを使用
        if 'total_iter' in opt['train']:
            self.max_iters = opt['train']['total_iter']
        elif 'total_epoch' in opt['train']:
            # train.py で計算された total_iters を使用（後で設定）
            self.max_iters = None  # 一時的に None
        else:
            raise KeyError('Either total_iter or total_epoch must be specified')
    ...
    def set_max_iters(self, max_iters):
        """Set max_iters after calculation."""
...
def init_wandb_logger(opt):
    """Initialize wandb logger and return the run object."""
    import wandb
