...
def paired_paths_from_folder(folders,
    ...
    Args:
        ...
        keys (list[str]): [input_key, gt_key], e.g. ['lq', 'gt'].
        filename_tmpl (str | None): Template for input filenames.
        metainfo (dict | None): Dataset metainfo.

    Returns:
        list[dict]: Returned keys are f'{input_key}_path' and f'{gt_key}_path'.
    """
    assert len(folders) == 2, (
        'The len of folders should be 2 with [input_folder, gt_folder]. '
        f'But got {len(folders)}')
    ...
    # フォルダ内のパス一覧（フルパス）
    gt_paths = sorted(scandir(gt_folder, full_path=True))
    input_paths = sorted(scandir(input_folder, full_path=True))

    # LQ 側: ファイル名（拡張子なし）→ パス一覧
    stem2paths = defaultdict(list)
    for p in input_paths:
        stem = osp.splitext(osp.basename(p))[0]
        stem2paths[stem].append(p)
    ...
    for gt_path in gt_paths:
        ...
        if filename_tmpl is not None:
            expected_stem = filename_tmpl.format(basename)
        else:
            expected_stem = basename

        # 1) 完全一致を優先
        if expected_stem in stem2paths:
            cand_paths = stem2paths[expected_stem]
        else:
            # 2) 無ければ prefix 一致（LQ だけ末尾にサフィックスが付いているケース）
            candidate_stems = [
                s for s in stem2paths.keys() if s.startswith(expected_stem)
            ]
            if not candidate_stems:
                raise AssertionError(f'{expected_stem} is not in input_paths.')
            chosen_stem = sorted(candidate_stems)[0]
            cand_paths = stem2paths[chosen_stem]

        # 同じ stem で複数あっても先頭を採用
        input_path = sorted(cand_paths)[0]
