...
class Registry:
    ...
    def register(self): return lambda x: x
...
class MambaIR(nn.Module):
    ...
    def __init__(self,
        ...
        num_in_ch = in_chans
        num_out_ch = kwargs.get('out_chans', 3)
        ...
        if in_chans == 3:
            ...
            self.mean = torch.zeros(1, num_in_ch, 1, 1)
    ...
    def forward(self, x):
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            # for image denoising / restoration
            x_first = self.conv_first(x)
            res = self.conv_after_body(self.forward_features(x_first)) + x_first
            # Skip connection: use only the first num_out_ch channels of input
            x = x[:, :self.num_out_ch, :, :] + self.conv_last(res)

        x = x / self.img_range + self.mean[:, :self.num_out_ch, :, :]
