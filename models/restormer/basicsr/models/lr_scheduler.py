...
def get_position_from_periods(iteration, cumulative_period):
    ...
    for i, period in enumerate(cumulative_period):
        if iteration <= period:
            return i
    # If iteration is greater than all cumulative periods, return the last index
    # so schedulers continue using the final period's behavior instead of
    # returning None which causes TypeError when indexing restart_weights.
    if len(cumulative_period) > 0:
        return len(cumulative_period) - 1
    # If no periods provided, default to index 0
    return 0
