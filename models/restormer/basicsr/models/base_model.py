...
class EarlyStopping:
    ...
    def __init__(self, patience=10, delta=0, mode='min', verbose=True):
        """
        Args:
            patience (int): How long to wait after last time validation improved.
            delta (float): Minimum change to qualify as an improvement.
            mode (str): 'min' for loss, 'max' for metrics like PSNR/SSIM.
    ...
    def __call__(self, current_score):
        ...
        improvement = (
            ...
            or (self.mode == 'max' and current_score > self.best_score + self.delta)
        )
        ...
        if improvement:
            ...
            self.counter = 0
            if self.verbose:
                logger.info(f"EarlyStopping: Improvement detected, best score updated to {self.best_score:.6f}")
        else:
            self.counter += 1
...
class BaseModel():
    ...
    def __init__(self, opt):
        ...
        # === Early Stopping ===
        es_opt = opt.get('early_stopping', None)
        if es_opt:
            self.early_stopping = EarlyStopping(
                patience=es_opt.get('patience', 10),
                delta=es_opt.get('delta', 0.0),
                mode=es_opt.get('mode', 'min'),
                verbose=True
            )
        else:
    ...
    def validation(self, dataloader, current_iter, tb_logger, save_img=False,
        ...
        """Validation function."""
        if self.opt['dist']:
            val_score = self.dist_validation(dataloader, current_iter, tb_logger, save_img, rgb2bgr, use_image)
        else:
            val_score = self.nondist_validation(dataloader, current_iter, tb_logger, save_img, rgb2bgr, use_image)

        # === Early Stopping 判定 ===
        if self.early_stopping is not None:
            # val_scoreはvalidation結果のmetricやlossを返すようにする
            if isinstance(val_score, dict):
                # lossなどのkeyを選んで使用
                monitored_value = val_score.get('val_loss') or list(val_score.values())[0]
            else:
                monitored_value = val_score
            self.early_stopping(monitored_value)

        return val_score

    def should_stop_training(self):
        """Return True if early stopping is triggered."""
