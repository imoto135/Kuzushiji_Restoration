...
class ImageCleanModel(BaseModel):
    ...
    def log_model_complexity(self):
        ...
        trainable_params = sum(p.numel() for p in self.net_g.parameters() if p.requires_grad)
        ...
        # Try to calculate FLOPs (if fvcore or thop is available)
        try:
            from fvcore.nn import FlopCountAnalysis
            inp_channels = self.opt['network_g'].get('inp_channels', 3)
            dummy_input = torch.randn(1, inp_channels, 128, 128).to(self.device)
            flops = FlopCountAnalysis(self.net_g, dummy_input)
            total_flops = flops.total()
            logger.info(f'  FLOPs: {total_flops:,} ({total_flops/1e9:.2f}G)')
            self.model_flops = total_flops
        except:
            try:
                from thop import profile
                inp_channels = self.opt['network_g'].get('inp_channels', 3)
                dummy_input = torch.randn(1, inp_channels, 128, 128).to(self.device)
                flops, params = profile(self.net_g, inputs=(dummy_input,), verbose=False)
                logger.info(f'  FLOPs: {flops:,} ({flops/1e9:.2f}G)')
                self.model_flops = flops
            except:
                logger.info(f'  FLOPs: Not available (install fvcore or thop to calculate)')
        ...
        if self.model_flops is not None:
            self.model_complexity['flops'] = self.model_flops
            self.model_complexity['flops_G'] = self.model_flops / 1e9
