...
def process_splits(dataset_root, splits=("train", "test", "val"), input_subdir="gt", output_subdir="lq"):
    """
    サポートする構成（自動判定）:

    A) dataset_root/{split}/{input_subdir}
       例: datasets/hiragana_full/test/gt

    B) dataset_root/{input_subdir}/{split}
       例: datasets/hiragana_full/gt/test

    出力は入力の構成に合わせて:
    - Aなら dataset_root/{split}/{output_subdir}
    - Bなら dataset_root/{output_subdir}/{split}
    """
    for split in splits:
        # まず A を試す
        in_dir_a = os.path.join(dataset_root, split, input_subdir)
        out_dir_a = os.path.join(dataset_root, split, output_subdir)

        # 次に B を試す
        in_dir_b = os.path.join(dataset_root, input_subdir, split)
        out_dir_b = os.path.join(dataset_root, output_subdir, split)

        if os.path.isdir(in_dir_a):
            in_dir, out_dir = in_dir_a, out_dir_a
            layout = "A: dataset_root/split/subdir"
        elif os.path.isdir(in_dir_b):
            in_dir, out_dir = in_dir_b, out_dir_b
            layout = "B: dataset_root/subdir/split"
        else:
            print(f"[SKIP] 入力ディレクトリが見つかりません: {in_dir_a} / {in_dir_b}")
            continue

        print("\n" + "-" * 60)
        print(f"Split: {split} ({layout})")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset-root", default="datasets/hiragana_full", help="データセットのルート")
    parser.add_argument("--splits", default="train,test,val", help="カンマ区切りの split")
    parser.add_argument("--input-subdir", default="gt", help="各 split の入力サブディレクトリ名")
    parser.add_argument("--output-subdir", default="lq", help="各 split の出力サブディレクトリ名")
