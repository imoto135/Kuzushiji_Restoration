    ...
    def _scan_paired_paths(self):
        ...
        for lq_path in lq_files:
            ...
            if os.path.exists(gt_path):
                ...
                if self.mask_folder is not None:
                    # Mask should match LQ filename exactly
                    mask_path = os.path.join(self.mask_folder, img_name)
                    if os.path.exists(mask_path):
                        entry['mask_path'] = mask_path
                    else:
                        # Warning: mask missing for this file?
                        # For now, we skip if mask is required but missing, or let it fail later?
                        # Safest is to only add if valid.
                         continue 
