...
def calculate_psnr(img1,
    ...
    Args:
        ...
        **kwargs: For other keyword arguments, e.g., use_mask, mask.
    ...
    def _psnr(img1, img2):
        ...
        if use_mask and mask is not None:
            ...
            if mask.shape[2] == 1 and img1.shape[2] > 1:
                mask_ = np.repeat(mask, img1.shape[2], axis=2)
            else:
                mask_ = mask
            
            diff = (img1 - img2) * mask_
            # Only count non-zero mask pixels (or sum of mask weights)
            valid_pixels = np.sum(mask_)
            if valid_pixels == 0:
                mse = 0
            else:
